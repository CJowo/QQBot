...
async def search(file, api_key: str):
    """
    ...
    async with aiohttp.ClientSession() as session:
        async with session.post(url='https://saucenao.com/search.php', data=multipartWriter, headers=headers) as resp:
...
async def analyze(text):
    ...
    return {
        ...
        'links': [
            ('', i) for i in result['data']['ext_urls']
        ],
        'data': data
    }
